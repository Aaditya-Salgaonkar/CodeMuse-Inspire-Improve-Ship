...
export default function CodeExplanationPage() {
  ...
  const generateExplanation = async () => {
    if (!code.trim()) return alert("Please enter some code to explain.");

    setLoading(true);
    const prompt = `
Explain this code in plain English in detail, describing what each part does and why:
\`\`\`\n${code}\n\`\`\`
`;

    try {
      const res = await fetch("/api/code-explanation", {
        method: "POST",
      ...
      setExplanation(data.content || "Failed to generate explanation.");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="font-sans bg-gradient-to-b from-black via-[#0a0a0a] to-black text-white overflow-x-hidden min-h-screen">
      ...
      {/* Hero Section */}
      <section className="relative w-full mt-10 h-70 flex items-center justify-center text-center overflow-hidden">
        <Particles
          options={{
            fpsLimit: 60,
            interactivity: {
        ...
        >
          ...
          <p className="mt-4 text-lg md:text-xl max-w-xl mx-auto text-gray-300 leading-relaxed">
            Paste your code below and get detailed plain-English explanations instantly, with AI-powered clarity.
          </p>
        </motion.div>
      </section>
        ...
        {explanation && (
          ...
          >
            <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-indigo-900 max-h-[500px] overflow-auto prose prose-invert">
              <h3 className="text-xl font-semibold mb-4 text-indigo-300">Explanation Preview</h3>
