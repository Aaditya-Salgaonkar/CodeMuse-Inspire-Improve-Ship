...
export default function BugDetectionPage() {
  ...
  const detectBugs = async () => {
    ...
    const prompt = `
Analyze the following code and detect any potential bugs, logical errors, or runtime issues.
Provide detailed explanations for each issue and suggest fixes:
\`\`\`\n${code}\n\`\`\`
`;

    try {
      const res = await fetch("/api/bug-detection", {
        method: "POST",
      ...
      setBugReport(data.content || "No issues detected or failed to analyze.");
    } catch (err) {
      console.error(err);
      setBugReport("Failed to analyze code for bugs.");
    }
      ...
      <section className="relative w-full mt-10 h-80 flex items-center justify-center text-center overflow-hidden">
        ...
        >
          ...
          <p className="mt-4 text-lg md:text-xl max-w-xl mx-auto text-gray-300 leading-relaxed">
            Automatically detect potential bugs, logical errors, and runtime issues in your code. Get clear explanations and suggested fixes.
          </p>
        </motion.div>
      </section>
      ...
      <section className="pb-10 px-6 max-w-5xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            {bugReport && (
              <button
                onClick={() => copyToClipboard(bugReport)}
        ...
        {bugReport && (
          ...
          >
            <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-indigo-900 max-h-[500px] overflow-auto prose prose-invert">
              <h3 className="text-xl font-semibold mb-4 text-indigo-300">Bug Report Preview</h3>
              <ReactMarkdown>{bugReport}</ReactMarkdown>
            </div>
